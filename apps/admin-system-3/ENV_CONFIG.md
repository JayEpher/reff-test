...
UmiJS v3 通过 `UMI_ENV` 环境变量来选择对应的 `.env` 文件：
...
export const getUserList = () => {
  return fetch(`${config.apiBaseUrl}/api/users`).then(res => res.json());
};
```
...
export default {
  ...
  effects: {
    *fetchUsers(_, { call, put }) {
      const response = yield call(fetch, `${config.apiBaseUrl}/users`);
      // ...
    },
  },
};
